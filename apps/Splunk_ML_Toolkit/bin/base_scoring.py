...
from util.scoring_util import (
    ...
    get_and_check_fields_one_array,
    add_default_params,
...
class ClassificationScoringMixin(object):
    ...
    def __init__(self, options):
        ...
        self.params, self.actual_fields, self.predicted_fields, self._meta_params = self.handle_options(options)
    ...
    def load_scoring_function_with_options(options):
        ...
        scoring_function = scoring_util.load_scoring_function(scoring_module_name, options.get('scoring_name'))
    ...
    def handle_options(self, options):
        ...
        actual_fields, predicted_fields = get_and_check_fields_two_2d_arrays(
    ...
    def prepare_input_data(self, df, actual_fields, predicted_fields, options):
        ...
        actual_df, predicted_df = prepare_classification_scoring_data(
            df, actual_fields, predicted_fields, options.get('mlspl_limits')
        )
        ...
        if self._meta_params.get('class_variable_headers') is not None:
            self.params['labels'] = get_union_of_field_values(categorical_actual_df, predicted_df)
    ...
    def score(self, df, options):
        ...
        result = [self.scoring_function(a, p, **self.params) for a, p in zip(actual_array.T, predicted_array.T)]
...
class ROCMixin(ClassificationScoringMixin):
    ...
    def check_params_with_data(self, actual_df, predicted_df):
        ...
        for f, series in actual_df.iteritems():
            warn_multiclass_series(series, multiclass_warn_msg.format(f, self._meta_params['pos_label']))
            assert_pos_label_in_series(series, self._meta_params['pos_label'], default_pos_label='1')
    ...
    def prepare_input_data(self, df, actual_fields, predicted_fields, options):
        ...
        actual_df, predicted_df = prepare_classification_scoring_data(
            df, actual_fields, predicted_fields, options.get('mlspl_limits'), limit_predicted_fields=False
        )
...
class SingleArrayScoringMixin(object):
    ...
    def handle_options(self, options):
        ...
        a_fields = get_and_check_fields_one_array(options, self.scoring_name)
    ...
    def prepare_and_check_data(df, fields, params):
        ...
        Args:
            df (pd.dataframe): input dataframe
            fields (list): the ordered fields which comprise the array
    ...
    def score(self, df, options):
        ...
        a_df, _meta_params = self.prepare_and_check_data(df, self.variables, self.params)

        a_array = a_df.values

        if a_array.ndim == 2 and a_array.shape[1] == 1:
            a_array = a_array.reshape(-1,)  # explicitly convert to 1d vector
...
class DoubleArrayScoringMixin(object):
    ...
    def handle_options(self, options):
        ...
        a_fields, b_fields = get_and_check_fields_two_2d_arrays(options, self.scoring_name)
    ...
    def prepare_and_check_data(self, df, a_fields, b_fields, params):
        ...
        Args:
            df (pd.dataframe): input dataframe
            a_fields (list): the ordered fields which comprise a_array
            b_fields (list): the ordered fields which comprise b_array
            params (dict): input parameters
    ...
    def score(self, df, options):
        ...
        a_df, b_df, _meta_params = self.prepare_and_check_data(df, self.a_fields, self.b_fields, self.params)

        a_array = a_df.values
        b_array = b_df.values

        # If single-fields passed, convert to 1d arrays for scoring methods such as Wilcoxon
        if (a_array.ndim == 2 and a_array.shape[1] == 1) and (b_array.ndim == 2 and b_array.shape[1] == 1):
...
class TSAStatsToolsMixin(SingleArrayScoringMixin):
    ...
    def load_scoring_function_with_options(options):
        ...
        scoring_function = load_scoring_function(scoring_module_name, options.get('scoring_name'))
        return scoring_function
    ...
    def handle_options(self, options):
        ...
        if len(a_fields) != 1 or len(b_fields) != 0:
            msg = ('Syntax error: scoring method "{}" operates on a single field '
...
class RegressionScoringMixin(object):
    ...
    def handle_options(self, options):
        ...
        return params, actual_fields, predicted_fields
    ...
    def convert_param_types(params):
        ...
        converted_params = validate_param_from_str_list(converted_params, 'multioutput', valid_multioutput_vals)
        return converted_params

    def prepare_and_check_data(self, df, a_fields, b_fields):
        """Perform pre-processing on data and check params against data.
         ...
         Args:
             df (pd.dataframe): input dataframe
             a_fields (list): requested fields comprising predicted df
             b_fields (list): requested fields comprising actual df
         ...
         """
        actual_df, predicted_df = prepare_statistical_scoring_data(df, a_fields, b_fields)

        _meta_params = {
            'field_identifiers': get_field_identifiers(actual_df, "Actual field", predicted_df, "Predicted field"),
        }
    ...
    def score(self, df, options):
        ...
        Returns:
            df_output (pd.dataframe): output dataframe
        """
        actual_array, predicted_array, _meta_params = self.prepare_and_check_data(
...
class ClusteringScoringMixin(object):
    ...
    def handle_options(self, options):
        ...
        if len(label_field) != 1 or len(feature_fields) == 0:
            msg = 'Value error: silhouette_score requires fields to be specified as ' \
                  '"..| score silhouette_score <label_field> against <feature_f1> <feature_f2> ... <feature_fn>"'
            raise RuntimeError(msg)

        if label_field[0] in feature_fields:
            msg = 'Value error: label field "{}" found in feature fields.'
            raise RuntimeError(msg.format(label_field[0]))

        feature_fields = remove_duplicate_fields_and_warn(feature_fields)
    ...
    @staticmethod
    def prepare_and_check_data(df, label_field, feature_fields):
        """Perform pre-processing on data and check params against data.
        Prepare data and validate params prior to scoring.

        Args:
            df (pd.dataframe): input dataframe
            label_field (list): predicted cluster field
            feature_fields (list): requested fields

        Returns:
            feature_df (pd.dataframe): preprocessed feature_df dataframe
            labels_df (pd.dataframe): categorical cluster labels
        """
        feature_df, label_df = prepare_clustering_scoring_data(df, label_field, feature_fields)
        ...
        if not 1 < float(labels_cardinality) < len(feature_df):
            msg = ("Value error: silhouette_score requires the number of label-classes to be greater than 1 "
    ...
    def score(self, df, options):
        ...
        feature_df, label_df = self.prepare_and_check_data(df, self.label_field, self.feature_fields)
