...
def validate_json(json_data, schema_file):
    ...
    try:
        ...
        raise RuntimeError("Failed to check JSON schema in {}: {}".format(schema_file, e.message))
    except ValidationError as e:
        raise RuntimeError("Unable to validate data against json schema in {}: {}, {}, {}, {}, {}".format(
            schema_file, e.message, e.context, list(e.path), list(e.schema_path), e.cause))
