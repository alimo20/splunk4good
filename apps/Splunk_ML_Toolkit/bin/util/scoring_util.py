...
def load_scoring_function(module_name, function_name):
    ...
    try:
        ...
        cexc.log_traceback()
        err_msg = ('Could not load scoring method "{}" -- double check that the '
...
def warn_multiclass_series(series, msg=None):
    ...
    if msg is None:
        msg = 'Expected field "{}" to be binary, but found {} classes.'.format(series.name, series_cardinality)
...
def convert_df_to_true_binary(df, pos_label):
    ...
    for j, (f_name, series) in enumerate(df.iteritems()):
...
def assert_nonempty_arrays(a_array, b_array=None, a_array_alias='a_array', b_array_alias='b_array'):
    ...
    if b_array is None:
        if a_array.size == 0:
            raise RuntimeError('Value error: No valid numerical fields exist for scoring method.')
    else:
...
def warn_on_same_fields(a_fields, b_fields=None):
    ...
    msg = ('Found duplicate field(s): {}. Comparing the same fields can '
           'result in ambiguous output for some scoring methods.')

    if b_fields is None:
        unique_fields = {}.fromkeys(a_fields).keys()
        ...
        if len(a_fields) == len(b_fields):
            ...
            duplicate_fields = [a_fields[i] for i in range(len(a_fields)) if a_fields[i] == b_fields[i]]
        else:
...
def validate_param_from_str_list(params, param_name, acceptable_values):
    ...
    if param_value not in acceptable_values:
        ...
        messages.error(msg.format(param_name, ', '.join(acceptable_values), param_name, param_value))
...
def drop_categorical_fields_and_warn(df, requested_fields=None):
    ...
    scols = [f for (f, series) in df.iteritems() if not series_type_is_numeric(series)]
    ...
    if len(requested_categorical_fields) > 0:
        cexc.messages.warn('Dropping categorical fields: {}.'.format(', '.join(requested_categorical_fields)))
...
def get_and_check_fields_one_array(options, scoring_name):
    """ Get fields from options and validate for single array

    - Removes duplicates from a_fields and warns on their removal
    ...
    Returns:
        fields (list): list containing requested fields with duplicates removed
    ...
    if len(a_fields) == 0 or len(b_fields) != 0:
        msg = 'Syntax error: expected " | score {} <field_1> <field_2> ... <field_n>."'
        raise RuntimeError(msg.format(scoring_name))

    # Remove and warn on duplicates
    a_fields = remove_duplicate_fields_and_warn(a_fields)
    return a_fields


def get_and_check_fields_two_1d_arrays(options, scoring_name, a_field_alias='a_field', b_field_alias='b_field'):
    """ Get fields from options and validate for 1d comparisons.
...
def get_and_check_fields_two_2d_arrays(options, scoring_name, a_field_alias='a_field', b_field_alias='b_field'):
    """ Get fields from options and validate for 2d comparisons.
    ...
    if (n1 == 0 or n2 == 0) or (n1 != n2 and n1 != 1):
        msg = 'Syntax error: expected " | score {scoring_name} <{a_field_alias}_1> ... <{a_field_alias}_n> ' \
              'against <{b_field_alias}_1> ... <{b_field_alias}_n>"'

        raise RuntimeError(msg.format(
            scoring_name=scoring_name, a_field_alias=a_field_alias, b_field_alias=b_field_alias)
        )
    return a_fields, b_fields
...
def assert_numeric_type_df(df):
    ...
    non_numeric_fields = [f for (f, series) in df.iteritems() if not series_type_is_numeric(series)]
...
def remove_duplicate_fields_and_warn(fields):
    ...
    duplicates = OrderedDict.fromkeys([i for i in fields if fields.count(i) > 1]).keys()
    if len(duplicates) > 0:
        cexc.messages.warn('Removing duplicate fields: {}.'.format(', '.join(duplicates)))
    fields = OrderedDict.fromkeys(fields).keys()
    return fields
...
def prepare_classification_scoring_data(
    ...
    cardinality_error_msg = 'Value error: cardinality of {} field "{}" exceeds limit of {}.'
    ...
    for f_a, series_a in df[list(set(actual_fields))].iteritems():
        if series_a.nunique() > max_distinct_cat_values:
            raise RuntimeError(cardinality_error_msg.format('actual', f_a, max_distinct_cat_values))

    # If applicable, Check that the cardinality of predicted fields is within limits
    if limit_predicted_fields:
        for f_p, series_p in df[list(set(predicted_fields))].iteritems():
            if series_p.nunique() > max_distinct_cat_values:
                raise RuntimeError(cardinality_error_msg.format('predicted', f_p, max_distinct_cat_values))
    ...
    assert_nonempty_arrays(actual_df, predicted_df, a_array_alias='actual_array', b_array_alias='predicted_array')
...
def assert_pos_label_in_series(series, pos_label, default_pos_label='1'):
    ...
    if series[series.isin([pos_label])].empty:
        msg = ('Value error: {} for pos_label not found in actual field "{}". '
               'Please specify a valid value for pos_label')
        if pos_label == default_pos_label:
            raise RuntimeError(msg.format('default value "{}"'.format(default_pos_label), series.name))
        else:
...
def assert_numeric_proba(predicted_df):
    ...
    """
    error_msg = ('Expected field "{}" to be numeric and correspond to probability estimates or confidence intervals '
                 'of the positive class, but field contains non-numeric events.')
    for f_name, series in predicted_df.iteritems():
...
def check_class_intersection(actual_df, predicted_df):
    ...
    """
    no_intersection_msg = ('There are no common classes between predicted ({}) and actual ({}) fields. '
    ...
    if actual_df.shape[1] == 1 and actual_df.shape != predicted_df.shape:
        _actual_df = pd.concat([actual_df] * predicted_df.shape[1], axis=1)  # Replicate for zipping
    else:
        _actual_df = actual_df

    # Iterate over the columns of actual/predicted dataframes; check the intersection of values
    for (f_a, s_a), (f_p, s_p) in zip(_actual_df.iteritems(), predicted_df.iteritems()):
        ...
        # Partial intersection
            ...
            if len(not_in_predicted) > 0:
                append_msg = ' actual field "{}" contains {} classes not in predicted field "{}".'
            ...
            if len(not_in_actual) > 0:
                append_msg = ' Predicted field "{}" contains {} classes not in actual field "{}".'
...
def convert_df_to_categorical(df, warn_on_float_series=False):
    ...
    """
    for j, (f, series) in enumerate(df.iteritems()):
        ...
        if warn_on_float_series:
            if series_is_float(series):
                cexc.messages.warn('Expected field "{}" to be categorical, but numeric field found. '
...
def initial_check_pos_label_and_average(params):
    ...
    params = validate_param_from_str_list(params, 'average', ['none', 'binary', 'macro', 'micro', 'weighted'])
    if average != 'binary' and params.get('pos_label', None) is not None:  # pos_label set by user
...
def check_pos_label_and_average_against_data(actual_df, predicted_df, params, default_pos_label='1'):
    ...
    """
    for (f_a, s_a), (f_p, s_p) in zip(actual_df.iteritems(), predicted_df.iteritems()):
        a_p_combined_cardinality = len(set(s_a.values).union(set(s_p.values)))
        if a_p_combined_cardinality > 2 and params['average'] == 'binary':
            msg = 'Value error: the combined cardinality of actual field "{}" and predicted field "{}" is greater ' \
        ...
        if params['average'] == 'binary':
            assert_pos_label_in_series(s_a, params.get('pos_label', default_pos_label), default_pos_label)
...
def check_alternative_param(params):
    ...
    """
    params = validate_param_from_str_list(params, 'alternative', ['none', 'two-sided', 'less', 'greater'])
    return params
...
def warn_order_not_preserved():
    ...
    """
    msg = 'Scoring method does not preserve order of events, and so event indices are arbitrary.'
    cexc.messages.warn(msg)
...
def check_alpha_param(alpha):
    ...
    if alpha <= 0 or alpha >= 1:
        raise RuntimeError("Value error: parameter 'alpha' must be between 0 and 1 (not inclusive).")
...
def update_with_hypothesis_decision(dict_results, alpha, null_hypothesis):
    ...
    try:
        ...
        update_dict = {"Test decision (alpha={})".format(alpha): decision_annotation(p_value, alpha, null_hypothesis)}
    except TypeError:
        # Multiple events
...
#################################
def prepare_clustering_scoring_data(df, label_field, feature_fields):
